---
name: dev-up
description: Start the envoyage umbrella (engine + cloud) — one Tilt that boots BOTH repos together on UI port 10462. The engine has no standalone Tilt; the umbrella scripts live in the sibling envoyage-cloud. ALWAYS use ./dev_up.sh, never `tilt up` directly.
...
./dev_up.sh            # Tilt UI on http://localhost:10462
```
...
- **Check for a stray umbrella Tilt first**: `ps aux | grep "[t]ilt up"` matching `--port 10462`; don't start a second.

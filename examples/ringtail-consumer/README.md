...
A small, self-contained example of consuming [rudder](../../README.md) from a
TypeScript product — the shape a ringtail engineer copies. Two halves:

- **`driver.ts`** — the **agent side**. Spawns `rudder serve --mcp --ws-port 8787`
...
- **`index.html` + `viewer.ts`** — the **UI side**. Connects to rudder's WS
...
```bash
# 1. Have `rudder` on PATH (cargo build --release -p rudder) or use npm:
#      export RUDDER_CMD="npx -y @immorterm/rudder"
...
`driver.ts` launches rudder with `--ws-port 8787`, so the viewer shows the live
...
- **MCP is native to a TS product** — no framework, just the MCP SDK stdio
  client spawning `rudder serve`.
- **Bring-your-own-mascot** — rudder emits `browser_cursor {x,y,action}`; the

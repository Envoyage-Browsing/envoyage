...
**rudder gives an AI agent a live, human-watchable browser. You bring the
mascot.**

This guide is for TypeScript products that want to embed rudder: let a coding
...
**Rocco** (its ringtail mascot). rudder draws nothing — it hands you the
...
You run `rudder serve` as a subprocess. It exposes two independent surfaces —
use one or both:
...
```bash
rudder serve --mcp --ws-port 8787
# or via npm:  npx @immorterm/rudder serve --mcp --ws-port 8787
```
...
rudder speaks MCP over **stdio**: newline-delimited JSON-RPC 2.0. The handshake
...
(`StdioClientTransport`) — spawn `rudder serve` as the command, and the SDK
...
Connect a plain WebSocket to `ws://127.0.0.1:<ws-port>`. rudder pushes JSON
...
Everything rudder emits is in **page CSS pixels**, and by rudder's scale-1
...
When rudder hits a Cloudflare/CAPTCHA check, an OAuth/sign-in screen, or a
...
rudder ships **no** cursor sprite. The `browser_cursor` event is the seam:
...
| `RUDDER_BROWSER_EVAL` | unset | `1` exposes the gated `browser_eval` tool. |
